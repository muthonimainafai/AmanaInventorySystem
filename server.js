...
async function initDb() {
  ...
  await run("ALTER TABLE chicken_sales ADD COLUMN feed_brand TEXT").catch(() => {});
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS cess_accounts_entries (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      date TEXT NOT NULL,
      description TEXT NOT NULL,
      quantity REAL NOT NULL,
  ...
  const salesKgStockFixV2 = await get("SELECT value FROM app_meta WHERE key = ?", ["sales_kg_stock_fix_v2"]);
  if (!salesKgStockFixV2 || salesKgStockFixV2.value !== "1") {
    const wmFix = await getRetailWeightKgByKeyMap();
    const allSkFix = await all("SELECT * FROM sales_kg");
    const groupsFix = new Map();
    for (const r of allSkFix) {
      const bk = resolveBrandKey(r.brand);
      const key = `${bk}|${normalizeFeedType(r.feed_type)}`;
      if (!groupsFix.has(key)) groupsFix.set(key, []);
      groupsFix.get(key).push(r);
    }
    for (const [, productRows] of groupsFix) {
  ...
  // v4: inventory follows bags opened (not bags completed). Deduct any still-open bags
  // that v3 left in stock so finishing remaining kg no longer requires a stock check.
  const salesKgStockFixV4 = await get("SELECT value FROM app_meta WHERE key = ?", ["sales_kg_stock_fix_v4"]);
  if (!salesKgStockFixV4 || salesKgStockFixV4.value !== "1") {
    const wmFix4 = await getRetailWeightKgByKeyMap();
    const allSkFix4 = await all("SELECT * FROM sales_kg");
    const groupsFix4 = new Map();
    for (const r of allSkFix4) {
      const bk = resolveBrandKey(r.brand);
      const key = `${bk}|${normalizeFeedType(r.feed_type)}`;
      if (!groupsFix4.has(key)) groupsFix4.set(key, []);
      groupsFix4.get(key).push(r);
    }
    for (const [, productRows] of groupsFix4) {
      if (!productRows.length) continue;
      const bk = resolveBrandKey(productRows[0].brand);
      const feedType = productRows[0].feed_type;
      const catalogSize = catalogBagSizeForFeed(bk, feedType);
      const opened = totalBagsOpenedFromKgSalesChronological(productRows, wmFix4);
      const completed = totalBagsCompletedFromKgSalesChronological(productRows, wmFix4);
      const extraOpen = opened - completed;
      if (extraOpen > 0) {
        try {
          await adjustInventoryBags({
            brand: bk,
          ...
          // skip if no matching inventory / not enough stock for the open bag
        }
      }
    }
    await run(`INSERT OR REPLACE INTO app_meta (key, value) VALUES (?, ?)`, ["sales_kg_stock_fix_v4", "1"]);
  }
...
function catalogBagSizeForFeed(brandKey, feedType) {
  const items = catalogForActiveTenant()[resolveBrandKey(brandKey)];
  if (!items) return 50;
...
function sortSalesKgRowsChronological(rows) {
...
/**
 * Bags taken from feed inventory for Sales Per Kg (explicit bag_opened + auto-opens).
 * Inventory is deducted when a bag is opened, not when the last kg from that bag is sold,
 * so staff can always record a sale of the remaining kg shown in Total(kgs) remaining.
 */
 ...
 * display. Feed Inventory deductions use totalBagsOpenedFromKgSalesChronological instead.
 */
...
function totalBagsDeductedLegacyDailyFloor(productRows, bagSize) {
  const byDay = new Map();
  for (const r of productRows) {
    const d = normalizeInventoryDate(r.date) || String(r.date || "").trim();
    byDay.set(d, (byDay.get(d) || 0) + Number(r.kg_sold || 0));
  }
  let total = 0;
  for (const kg of byDay.values()) {
    total += bagsFromTotalKg(kg, bagSize);
  }
  return total;
}

async function getSalesKgRowsForProduct(brandKey, rawFeedType) {
  const bk = resolveBrandKey(brandKey);
  const ftN = normalizeFeedType(rawFeedType);
  const rows = await all("SELECT * FROM sales_kg WHERE brand = ?", [bk]);
  return rows.filter((r) => normalizeFeedType(r.feed_type) === ftN);
}

async function countBagsOpenedForKgProduct(brandKey, rawFeedType) {
  const wm = await getRetailWeightKgByKeyMap();
  const rows = await getSalesKgRowsForProduct(brandKey, rawFeedType);
  return totalBagsOpenedFromKgSalesChronological(rows, wm);
}

/** @deprecated Prefer countBagsOpenedForKgProduct — inventory follows bags opened. */
async function countBagsCompletedForKgProduct(brandKey, rawFeedType) {
  return countBagsOpenedForKgProduct(brandKey, rawFeedType);
}
...
async function getRetailFeedLine(brandKey, feedType) {
  const canonFeed =
    (catalogForActiveTenant()[resolveBrandKey(brandKey)] || []).find(
...
async function assertEmployeeFeedSalePrices(req, res, mode, p) {
  ...
  const items = catalogForActiveTenant()[brandKey];
...
async function resolveEmployeeChickenFeedBundle(req, res, p, existingRow = null) {
  ...
  const brandRaw =
    p.feed_brand != null && String(p.feed_brand).trim() !== ""
      ? String(p.feed_brand).trim()
      : existingRow?.feed_brand != null && String(existingRow.feed_brand).trim() !== ""
        ? String(existingRow.feed_brand).trim()
        : "";
  const feedTypeRaw =
    p.feed_type != null && String(p.feed_type).trim() !== ""
      ? String(p.feed_type).trim()
      : existingRow?.feed_type != null && String(existingRow.feed_type).trim() !== ""
  ...
  if (bagQtyRaw === undefined || bagQtyRaw === null || String(bagQtyRaw).trim() === "") {
    if (existingRow != null && existingRow.feed_bag_qty != null && String(existingRow.feed_bag_qty).trim() !== "") {
      bagQtyRaw = existingRow.feed_bag_qty;
    } else {
      bagQtyRaw = 0;
    }
  }
  ...
  if (!Number.isFinite(bagQty) || bagQty < 0) {
    res.status(400).json({ error: "Feed quantity (bags) must be a whole number zero or greater." });
    return null;
  }
  const brandKey = resolveBrandKey(brandRaw);
  const catalog = catalogForActiveTenant();
  if (!catalog[brandKey] || !Array.isArray(catalog[brandKey]) || !catalog[brandKey].length) {
    res.status(400).json({ error: "Unknown feed brand for this site." });
    return null;
  }
  ...
  if (!typeOk) {
    res.status(400).json({ error: "Select a valid feed type for this brand." });
    return null;
  }
  ...
  if (!itemResolved) {
    res.status(400).json({
      error: "No Feed Inventory line for this brand and feed type. The owner must add stock first.",
    });
    return null;
  }
  const selling = Number(itemResolved.selling_price);
  if (!Number.isFinite(selling) || selling < 0) {
    res.status(400).json({ error: "Feed Inventory selling price is missing for this product." });
    return null;
  }
  ...
  return {
    feed_brand: storedBrand,
    feed_type: storedType,
    feed_bag_qty: bagQty,
    feed_line_total: lineTotal,
  };
}
...
app.put("/api/expenditure/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  const id = Number(req.params.id);
  const existing =
    req.user.role === "owner"
      ? await get("SELECT * FROM employee_expenditure WHERE id = ?", [id])
      : await get("SELECT * FROM employee_expenditure WHERE id = ? AND created_by = ?", [id, req.user.username]);
...
app.delete("/api/expenditure/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  const result =
    req.user.role === "owner"
      ? await run("DELETE FROM employee_expenditure WHERE id = ?", [Number(req.params.id)])
      : await run("DELETE FROM employee_expenditure WHERE id = ? AND created_by = ?", [
...
app.get("/api/cess-accounts", auth, allowRoles("owner", "employee"), async (req, res) => {
  const rows =
    req.user.role === "owner"
      ? await all("SELECT * FROM cess_accounts_entries ORDER BY id DESC")
      : await all("SELECT * FROM cess_accounts_entries WHERE created_by = ? ORDER BY id DESC", [req.user.username]);
  res.json(rows);
});

app.post("/api/cess-accounts", auth, allowRoles("owner", "employee"), async (req, res) => {
  const p = req.body || {};
  const dateCanon = normalizeInventoryDate(p.date);
  if (!dateCanon) return res.status(400).json({ error: "Invalid date. Use DD/MM/YYYY." });
  const description = String(p.description || "").trim();
...
app.put("/api/cess-accounts/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  const id = Number(req.params.id);
  const existing =
    req.user.role === "owner"
      ? await get("SELECT * FROM cess_accounts_entries WHERE id = ?", [id])
      : await get("SELECT * FROM cess_accounts_entries WHERE id = ? AND created_by = ?", [id, req.user.username]);
  if (!existing) return res.status(404).json({ error: "Record not found." });
  const p = req.body || {};
  const dateCanon = normalizeInventoryDate(p.date);
  if (!dateCanon) return res.status(400).json({ error: "Invalid date. Use DD/MM/YYYY." });
...
app.delete("/api/cess-accounts/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  const result =
    req.user.role === "owner"
...
app.post("/api/sales/kg", auth, allowRoles("owner", "employee"), async (req, res) => {
  ...
  const isThrough = throughParty != null;
  const nowIso = new Date().toISOString();
  const weightMapForKg = await getRetailWeightKgByKeyMap();
  const effBagSizeKg = effectiveKgPerOpenedBagForDisplay(weightMapForKg, brandKey, p.feed_type);

  let allSkBrand = [];
  let wmCoerce = weightMapForKg;
  if (req.user.role === "employee") {
...
app.put("/api/sales/kg/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  ...
  const weightMapPut = await getRetailWeightKgByKeyMap();
  const effBagSizePut = effectiveKgPerOpenedBagForDisplay(weightMapPut, brandKey, p.feed_type);
  ...
  const productKeysForPut = sameItem
    ? [{ brandKey, feedType: p.feed_type, catalogBagSize: defaultBagSize }]
    : [
  ...
  try {
    for (const prod of productKeysForPut) {
      ...
      const before = totalBagsOpenedFromKgSalesChronological(rowsBefore, weightMapPut);
      let rowsAfter;
      if (sameItem && prod.brandKey === brandKey && normalizeFeedType(prod.feedType) === normalizeFeedType(p.feed_type)) {
        ...
        rowsAfter = rowsBefore.filter((r) => Number(r.id) !== idNum);
      } else if (
        !sameItem &&
        prod.brandKey === brandKey &&
        normalizeFeedType(prod.feedType) === normalizeFeedType(p.feed_type)
      ) {
        rowsAfter = [
          ...rowsBefore,
          {
      ...
      const after = totalBagsOpenedFromKgSalesChronological(rowsAfter, weightMapPut);
...
app.delete("/api/sales/kg/:id", auth, allowRoles("owner", "employee"), async (req, res) => {
  ...
  const isThroughDelete = normalizeThroughParty(current.through_party) != null;
  const idNum = Number(req.params.id);
  try {
    ...
    const beforeDelete = totalBagsOpenedFromKgSalesChronological(rowsBeforeDelete, weightMapDel);
    const rowsAfterDelete = rowsBeforeDelete.filter((r) => Number(r.id) !== idNum);
    const afterDelete = totalBagsOpenedFromKgSalesChronological(rowsAfterDelete, weightMapDel);
    ...
    if (invDeltaDelete !== 0) {
      await adjustInventoryBags({
        ...
        feedType: current.feed_type,
        bagSize: defaultBagSize,
        deltaBags: -invDeltaDelete,
        recordProfit: !isThroughDelete,
      });
    }
  } catch (error) {
    return res.status(400).json({ error: error.message });
...
async function reverseAndDeleteFeedSalesForCreator(creator) {
  ...
  const kgProductsAffected = new Map();
  for (const row of skRowsToClear) {
    const brandKey = resolveBrandKey(row.brand);
    const key = `${brandKey}|${normalizeFeedType(row.feed_type)}`;
    if (!kgProductsAffected.has(key)) {
      kgProductsAffected.set(key, { brandKey, feedType: row.feed_type });
    }
  }
  const kgConsumedBeforeDelete = new Map();
  for (const [key, prod] of kgProductsAffected) {
    kgConsumedBeforeDelete.set(key, await countBagsOpenedForKgProduct(prod.brandKey, prod.feedType));
  }
  for (const row of skRowsToClear) {
  ...
  for (const [key, prod] of kgProductsAffected) {
    const before = kgConsumedBeforeDelete.get(key) || 0;
    const bagSize = catalogBagSizeForFeed(prod.brandKey, prod.feedType);
    const creatorRows = skRowsToClear.filter(
      (r) =>
        resolveBrandKey(r.brand) === prod.brandKey &&
        normalizeFeedType(r.feed_type) === normalizeFeedType(prod.feedType)
    );
    ...
    const wm = await getRetailWeightKgByKeyMap();
    const after = totalBagsOpenedFromKgSalesChronological(remainingRows, wm);
...
async function reassignSalesKgRowBrand(idNum, newBrandRaw, options = {}) {
  ...
  for (const prod of productKeys) {
    const rowsBefore = await getSalesKgRowsForProduct(prod.brandKey, prod.feedType);
    const before = totalBagsOpenedFromKgSalesChronological(rowsBefore, weightMap);
    let rowsAfter;
    if (
      prod.brandKey === currentBrandKey &&
      normalizeFeedType(prod.feedType) === normalizeFeedType(current.feed_type)
    ...
    const after = totalBagsOpenedFromKgSalesChronological(rowsAfter, weightMap);
    const invDelta = after - before;
    if (invDelta === 0) continue;
    await adjustInventoryBags({
      brand: prod.brandKey,
